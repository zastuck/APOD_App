...
def APOD_dialog():
    ...
    <div style="position: relative; display: flex; justify-content: center; width: 100%;">
        ...
        <style>
            ...
            <h4 style="margin: 0 0 1.2vw 0; font-size: clamp(0.9rem, 2.5vw, 1.4rem);">{apod['title']}</h4>
            <h6 style="margin: 0 0 1vw 0; font-size: clamp(0.7rem, 1.5vw, 1rem);">{pub_date.strftime("%A, %B %d, %Y")}</h6>
            <p class="apod-explanation" style="margin: 0 0 1vw 0; font-size: clamp(0.6rem, 1.2vw, 0.85rem);">{apod['explanation']}</p>
            <p class="apod-explanation" style="margin: 0; font-size: clamp(0.6rem, 1.2vw, 0.85rem);">{apod['credit']}</p>
        </div>
    </div>
    """,
    unsafe_allow_html=True
